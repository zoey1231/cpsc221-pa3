...
/**
	...
	* of the tied locations (rightmost, or bottommost, depending on 
	* splitting dimension).**
    * 
...
twoDtree::twoDtree(PNG &imIn)
{
	...
	bool vert = true; // modification: root is at level 0 not level 1
...
twoDtree::Node *twoDtree::buildTree(stats &s, pair<int, int> ul, pair<int, int> lr, bool vert)
{
	...
	if (ul.first != lr.first || ul.second != lr.second)
	{
		...
		* lower right corner.*/
		...
		long left_sum, right_sum, score, best_score;
		...
		else if (vert)
		{
			...
			for (split = ul.first; split < lr.first; split++)
			{
				...
				// make new pairs using current splitting line
				left_lr = make_pair(split, lr.second);
				right_ul = make_pair(split + 1, ul.second); //modification：我把这里从lr.second改成了ul.second
		...
		else if (!vert)
		{
			...
			for (split = ul.second; split < lr.second; split++)
			{
				...
				// make new pairs using current splitting line
				left_lr = make_pair(lr.first, split);
				right_ul = make_pair(ul.first, split + 1); //modification：我把这里从lr.first改成了ul.first
...
void twoDtree::recurRender(PNG &im, twoDtree::Node *rt)
{
	...
	// if rt is a leaf node, draw avg color into the pixel
...
int twoDtree::idealPrune(int leaves)
{
	...
	//tolerance越小，leaves越大
	//use binary search
	return recuridealPrune(0, 10000, leaves);
}

int twoDtree::recuridealPrune(int l, int r, int leaves)
{
	if (r <= l)
		return l;

	int mid = (l + r) / 2;
	int actual_leaves = pruneSize(mid);
	if (actual_leaves == leaves)
		return mid;
	else if (actual_leaves > leaves) //tolerance is too small,search in right half
		return recuridealPrune(mid + 1, r, leaves);
	else
		return recuridealPrune(l, mid - 1, leaves);
}
/**	
...
int twoDtree::pruneSize(int tol)
{
	...
	recurPruneSize(tol, root, c, check_tol(root, tol, root->avg), false);
	//recurPruneSize(tol, root, c, true, false);
	return c;
}

int twoDtree::count_subtree_node_num(twoDtree::Node *rt)
{
	return recurcount_node_num(rt);
}
int twoDtree::recurcount_node_num(twoDtree::Node *rt)
{
	unsigned int count = 1;
	if (rt->left != NULL)
	{
		count += recurcount_node_num(rt->left);
	}
	if (rt->right != NULL)
	{
		count += recurcount_node_num(rt->right);
	}
	return count;
}
/**
...
void twoDtree::recurPruneSize(int tol, twoDtree::Node *rt, int &c, bool p, bool chk)
{
	...
	// } // if not, call recursion
	// else
	// 	recurPruneSize(tol, rt->left, c, true, false);
	...
	// } // if not, call recursion
	// else
	// 	recurPruneSize(tol, rt->right, c, true, false);

	// if (p && !chk)
	// 	c++; // count 1 if rt is ready to be pruned (so rt is the only leaf)
	// else
	// 	c += 2; // count 2 if rt is not to be pruned (rtinal tree is full)

	// chk = true;

	//if have been checked already or rt == NULL,directly return.
	if (rt == NULL || chk)
		return;

	//if we should prune the current node, we remove all of its children in prune,
	//so this node now become a leaf
	if (p)
	{
		c++;
		return;
	}
	//else check the node's left and right child recursively
	else
	{
		recurPruneSize(tol, rt->left, c, check_tol(rt->left, tol, rt->left->avg), false);
		recurPruneSize(tol, rt->right, c, check_tol(rt->right, tol, rt->right->avg), false);
	}
	chk = true;
}
...
void twoDtree::prune(int tol)
{
	...
	recurPrune(tol, root, check_tol(root, tol, root->avg), false);
}
...
// 	} // if not, call recursion
// 	else
// 		recurPrune(tol, rt->left, true, false);
...
// 	} // if not, call recursion
// 	else
// 		recurPrune(tol, rt->right, true, false);
...
void twoDtree::recurPrune(int tol, twoDtree::Node *rt, bool p, bool chk)
{
	...
	// //if have been checked already or rt == NULL,directly return.
	// if (rt == NULL || chk) return;

	// //if come across a leaf node
	// if (rt->left == NULL && rt->right == NULL)
	// {
	// 	//if color difference is within the tolerance
	// 	if(check_tol(rt, tol, rt->avg)) {
	// 		p = true;
	// 	} else{
	// 		p = false;
	// 	}
	// } else {
	// 	recurPrune(tol,rt->left,p,chk);
	// 	if (p == true) left_prune = true;
	// 	recurPrune(tol, rt->right, p, chk);
	// 	if(p == true) right_prune = true;
	// 	//if both left and right subtree return true from p
	// 	// then remove the left and right subtrees, and return true for p
	// 	if(!chk && left_prune && right_prune) {
	// 		delete (rt->left);
	// 		rt->left = NULL;
	// 		delete (rt->right);
	// 		rt->right = NULL;
	...
	//if we should prune the current node, we remove both of its children
	if (p)
	{
		if (rt->left != NULL)
		{
	...
	{
		recurPrune(tol, rt->left, check_tol(rt->left, tol, rt->left->avg), false);
		recurPrune(tol, rt->right, check_tol(rt->right, tol, rt->right->avg), false);
	}
}
//return ture if the all leaves in subtee rooted at *rt has a color distance <= tolerance
bool twoDtree::check_tol(twoDtree::Node *rt, int tol, RGBAPixel avg)
{
	if (rt == NULL)
		return true;
	//base case:if *rt is a leaf node
	if (rt->left == NULL && rt->right == NULL)
	{
		if ((rt->avg.r - avg.r) * (rt->avg.r - avg.r) + (rt->avg.g - avg.g) * (rt->avg.g - avg.g) + (rt->avg.b - avg.b) * (rt->avg.b - avg.b) <= tol)
		{
			return true;
		}
		else
	...
	//recursive check
	return check_tol(rt->left, tol, avg) && check_tol(rt->right, tol, avg);
}
/**
